...
public class NoticeUpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		Board board = service.findNoticeByNo(boardNo, false);
		
		req.setAttribute("board", board);
		req.getRequestDispatcher("/views/board/update2.jsp").forward(req, resp);
	}
...
//			board.setWriterId(writer);
			...
			int result = service.nsave(board); // db에 게시글 저장
			
			if(result <= 0 || loginUser.getGrade()==1 == false) {
				sendCommonPage(req, resp, "수정 권한이 없습니다.", "/board/notice");
				return;
			}
			sendCommonPage(req, resp, "게시글이 수정되었습니다.", "/board/notice");
		} catch (Exception e) {
			e.printStackTrace();
			sendCommonPage(req, resp, "게시글을 수정할 수 없습니다. (code=102)", "/board/notice");
		}
	}
