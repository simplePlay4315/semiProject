...
//			String writer = req.getParameter("writer");
			...
			int result = service.nsaveReply(reply);
			
			if(result <= 0 ) {
				sendCommonPage(req, resp, "댓글을 등록할 수 없습니다. (code=101)", "/board/view2?boardNo="+boardNo);
				return;
			}
			sendCommonPage(req, resp, "댓글이 등록되었습니다.", "/board/view2?boardNo="+boardNo);
		} catch (Exception e) {
			e.printStackTrace();
