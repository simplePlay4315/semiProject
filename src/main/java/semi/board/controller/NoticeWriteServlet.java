	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			if(loginUser != null) { // 정상적인 흐름
				req.getRequestDispatcher("/views/board/write2.jsp").forward(req, resp);
				return;
			}
		} catch (Exception e) {}
...
//			System.out.println(writer);
			if (loginUser.getGrade() != 1) { //
				sendCommonPage(req, resp, "글쓰기 권한이 없습니다.", "/board/notice");
				return;
			}
			...
			if(result <= 0 ) {
				sendCommonPage(req, resp, "게시글을 등록할 수 없습니다. (code=101)", "/board/notice");
				return;
			}
			sendCommonPage(req, resp, "게시글이 등록되었습니다.", "/board/notice");
		} catch (Exception e) {
			e.printStackTrace();
			sendCommonPage(req, resp, "게시글을 등록할 수 없습니다. (code=102)", "/board/notice");
		}
	}
