...
public class NoticeDAO {
	...
	public int getNoticeCount(Connection conn) {
		...
		ResultSet rs = null;
		String query =  "SELECT COUNT(*) FROM NOTICE_TBL WHERE STATUS='Y'";
		try {
	...
	// 게시물의 정보를 가져오는 메소드
	public List<Board> nfindAll(Connection conn, PageInfo pageInfo) {
		...
		String query = "SELECT RNUM, NO, TITLE, USER_ID, CREATE_DATE, MODIFY_DATE, ORIGINAL_FILENAME, READCOUNT, STATUS  "
				...
				+ "    FROM ( "
				+ "        SELECT  N.NO, N.TITLE, U.USER_ID, TO_CHAR(N.CREATE_DATE,'hh24:mi') as CREATE_DATE, N.MODIFY_DATE, N.ORIGINAL_FILENAME, N.READCOUNT, N.STATUS "
				+ "        FROM NOTICE_TBL N JOIN USER_TBL U ON(N.WRITER_NO = U.USER_NO) "
		...
		try {
			...
			while(rs.next()) {
				...
				board.setCreateDT(rs.getString("CREATE_DATE"));
...
//				board.setModifyTime(rs.getTime("MODIFY_DATE"));
	...
	public int insertNotice(Connection conn, Board board) {
		int result = 0;
		PreparedStatement pstmt = null;
		String query = "INSERT INTO NOTICE_TBL VALUES(NOTICE_SEQ.NEXTVAL,?,?,?,?,?,DEFAULT,DEFAULT,DEFAULT,DEFAULT)";
		try {
			pstmt = conn.prepareStatement(query);
	...
	// 게시글을 가져오는 객체
	public Board findNoticeByNo(Connection conn, int boardNo) {
		Board board = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String query = "SELECT N.NO, N.TITLE, U.USER_ID, N.READCOUNT, N.ORIGINAL_FILENAME, N.RENAMED_FILENAME, N.CONTENT, "
				+ " TO_CHAR(N.CREATE_DATE,'yyyy.mm.dd hh24:mi') as CREATE_DATE, TO_CHAR(N.MODIFY_DATE,'yyyy.mm.dd hh24:mi') as MODIFY_DATE "
				+ "	FROM NOTICE_TBL N JOIN USER_TBL U ON(N.WRITER_NO = U.USER_NO) "
				+ "	WHERE N.STATUS = 'Y' AND N.NO = ? ";
		try {
			pstmt = conn.prepareStatement(query);
			...
			if(rs.next()) {
				...
				board.setCreateDT(rs.getString("CREATE_DATE"));
//				board.setCreateDT(rs.getString("MODIFY_DATE"));
//				board.setCreateTime(rs.getTime("CREATE_DATE"));
//				board.setModifyTime(rs.getTime("MODIFY_DATE"));
				board.setReplies(getnRepliesByNo(conn, boardNo));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}finally{
	...
	// 게시글 조회수 올려주는 쿼리
	public int updatenReadCount(Connection conn, Board board) {
		int result = 0;
		PreparedStatement pstmt = null;
		String query = "UPDATE NOTICE_TBL SET READCOUNT=? WHERE NO=?";
		try {
			pstmt = conn.prepareStatement(query);
	...
	public int nupdateStatus(Connection conn, int boardNo, String status) {
		int result = 0;
		PreparedStatement pstmt = null;
		String query = "UPDATE NOTICE_TBL SET STATUS=? WHERE NO=?";
		try {
			pstmt = conn.prepareStatement(query);
	...
	public int updateNotice(Connection conn, Board board) {
		int result = 0;
		PreparedStatement pstmt = null;
		String query = "UPDATE NOTICE_TBL SET TITLE=?,CONTENT=?,ORIGINAL_FILENAME=?,RENAMED_FILENAME=?, "
	...
	// 리플을 가져오는 메소드
	public List<Reply> getnRepliesByNo(Connection conn, int boardNo) {
	...
	// 리플 쓰기 기능
	public int insertnReply(Connection conn, Reply reply) {
		int result = 0;
		PreparedStatement pstmt = null;
	...
	public int deletenReply(Connection conn, int replyNo) {
		int result = 0;
		PreparedStatement pstmt = null;
