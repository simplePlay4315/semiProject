...
<%
	List<Reserve> list = (List<Reserve>) request.getAttribute("list");
                    ...
                    <i class="bi-chevron-right"></i>
                                ...
                                <ul class="nav nav-segment nav-fill mb-7" id="featuresTab" role="tablist">
                                    <li class="nav-item" role="presentation">
                                        <a class="nav-link" href="<%=path %>/myPage/resList" id="accountOrdersOne-tab" >예약 내역</a>
                                    </li>

                                    <li class="nav-item" role="presentation">
                                        <a class="nav-link active"  id="accountOrdersThree-tab" >취소 내역</a>
                                    </li>
                                </ul>
                                <!-- End Nav -->
                            </div>
                                ...
                                <div class="tab-pane fade show active" id="accountOrdersOne" role="tabpanel" aria-labelledby="accountOrdersOne-tab">
									<%if(list == null || list.size() == 0) {%>
                                    	...
                                    	<div class="text-center content-space-1">
                                        	...
                                        	<p class="card-text">취소 내역이 없습니다.</p>
                                    	</div>
                                    <!-- End Empty State -->
									<%}else { %>
                                            ...
                                            <div class="card-body">
                                                <div class="row">
                                                    <div class="col-6 col-md mb-3 mb-md-0">
                                                        ...
                                                        <small class="text-dark fw-semi-bold"><%=list.get(i).getResNo() %></small>
                                                    </div>
                                                    <!-- End Col -->

                                                    <div class="col-6 col-md mb-3 mb-md-0">
                                                        <small class="card-subtitle mb-0">낚시터 이름</small>
                                                        <small class="text-dark fw-semi-bold"><%=list.get(i).getFishing().getName() %></small>
                                                    </div>
                                                    <!-- End Col -->
                                                    ...
                                                    <div class="col-6 col-md">
                                                        ...
                                                        <small class="text-dark fw-semi-bold"><%=list.get(i).getResDate() %></small>
                                                    </div>
                                                    <!-- End Col -->
                                                </div>
                                                <!-- End Row -->
                                                ...
                                                <div class="row">
                                                    <div class="col-md-7">
                                                        <div class="row gx-2">
                                                            <div class="col">
                                                                <img style="height:225px; width: 400px;" src="<%=path %>/views/css/img/PLACE_<%=list.get(i).getFishing().getNo() % 18 %>.jpg">
                                                            </div>
                                                        </div>
                                                        <!-- End Row -->
                                                    </div>
                                                    ...
                                                    <div class="col-md-5 mt-5">
                                                        <div class="d-grid gap-2">
                                                        	<a></a>
                                                            <a style="margin-top: 20px;" class="btn btn-white btn-sm" href="<%=path%>/fishing/view?fishingNo=<%=list.get(i).getFishing().getNo()%>">
                                                                <i class="bi-people-fill small me-2"></i> 리뷰 보러가기
                                                            </a>
                                                            <a class="btn btn-primary btn-sm" onclick="deleteRes(<%=list.get(i).getResNo()%>)">
                                                                <i class="bi-x-circle small me-2"></i> 항목에서 지우기
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                            	...
                            	<%} %>
                                    ...
                                    <nav aria-label="Page navigation">
                                        ...
                                        <%if(pageInfo.getCurrentPage() != 1) {%>
                                        	<li class="page-item">
                                                <a class="page-link" href="<%=path %>/myPage/cancleList?page=<%=pageInfo.getPrvePage()%>" aria-label="Prev">이전</a>
                                            </li>
                                        <%} %>
                                            ...
                                            <%for (int i = pageInfo.getStartPage(); i <= pageInfo.getEndPage(); i++) {%>
                                            	...
                                            	<%}else { %>
                                            		<li class="page-item"><a class="page-link" href="<%=path %>/myPage/cancleList?page=<%=i%>"><%=i %></a></li>
                                            	<%} %>
                                            <%} %>
                                            <!-- <li class="page-item disabled"><a class="page-link" href="#">...</a></li> -->
                                            <%if(pageInfo.getCurrentPage() != pageInfo.getEndPage()) {%>
                                        	<li class="page-item">
                                                <a class="page-link" href="<%=path %>/myPage/cancleList?page=<%=pageInfo.getNextPage() %>" aria-label="Next">다음</a>
                                            </li>
                                        <%} %>
...
<script>
	function deleteRes(no){
		 if(confirm("정말로 항목에서 지우시겠습니까? (복구가 불가능합니다.)")) {
			location.replace("<%=path %>/fishing/reserve/delete?resNo=" + no);
		} 
	}
</script>
